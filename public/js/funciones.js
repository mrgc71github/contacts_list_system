...
function activarCalendario(idioma=null) {
  ...
  if (document.getElementById("VueloDestino").value != '' && document.getElementById("VueloOrigen").value != '') {
    ...
    $('#fecha_desde').datepicker( $.datepicker.regional[ local ] );
  } else {
    $('#fecha_desde').datepicker("hide");
    $('#fecha_desde').datepicker("destroy");
    if (idioma == 'en') {
...
function restringirFechaInicial() {
  fecha = document.getElementById("fecha_desde").value;
  $("#fecha_hasta").datepicker("option", "minDate", fecha);
  fecha2 = document.getElementById("fecha_hasta").value;
...
function activarDiasIda(fecha,dom, lun, mar, mie, jue, vie, sab, domv, lunv, marv, miev, juev, viev, sabv,tipo) {
  $('#fecha_desde').datepicker("destroy");
  $('#fecha_hasta').datepicker("destroy");
  $('#fecha_hasta').datepicker("hide");
  if(tipo=='NAC'){
    fecha_max='2019-06-30';
  }else{
  ...
  $(function() {
    $('#fecha_desde').datepicker({
      ...
      onClose: function(selectedDate) {
        $("#fecha_hasta").datepicker("option", "defaultDate", "minDate", selectedDate);
      },
...
function activarDiasVuelta(dom, lun, mar, mie, jue, vie, sab,tipo) {
  if(tipo=='NAC'){
    fecha_max='2020-03-31';
  }else{
    fecha_max='2020-06-30'
  }
  $(function() {
    $('#fecha_hasta').datepicker({
      dateFormat: 'yy-mm-dd',
...
function soloida(e) {
    ...
    if (document.getElementById("solo_ida").checked == true){
     ...
     /* e.departure_airport.value = "", //ORIGEN DEL VUELO
      e.arrival_airport.value = "", //DESTINO DEL VUELO*/
      /*e.departure_date.value = "", //FECHA DE IDA DEL VUELO*/
      e.arrival_date.value = "", //FECHA DE VUELTA DEL VUELO
...
function selectorMoneda(e) {
    if (document.getElementById("ves").checked == true){
      console.log("USD");
      e.moneda.value = "USD"
    }else{
        console.log("VES");
...
function enviar(e, a) {
  ...
  var dia_ida = 0,
    ...
    dia_ida = e.fecha_desde.value.substring(8, 10),
    mes_ida = e.fecha_desde.value.substring(5, 7),
    anio_ida = e.fecha_desde.value.substring(0, 4),
    //ASIGNO FORMATO A LA FECHA IDA 00-00-0000
    fecha_ida = dia_ida + '-' + mes_ida + '-' + anio_ida;

  if (e.fecha_hasta.value != '') {
    //SUBSTRING PARA SEPARAR LA FECHA VUELTA
    dia_vuelta = e.fecha_hasta.value.substring(8, 10),
      mes_vuelta = e.fecha_hasta.value.substring(5, 7),
      anio_vuelta = e.fecha_hasta.value.substring(0, 4),
  ...
  if (
    ...
    if (
    "" == e.localizador.value)
    return "en" == a ? emitir_mensaje("You must type your PNR") : emitir_mensaje("Debe ingresar el Localizador"), !1;
  if (
  ...
  //DESDE ESTE PUNTO SE EMPIEZAN A RECOGER LAS VARIABLES QUE SE ENVIARAN A LA URL POR POST
    ...
    //e.acceso.value = e.acceso.value, 
    //e.id_usuario.value = e.id_usuario.value, 
    //e.nombre.value = e.nombre.value,
    //e.id_agencia.value = e.id_agencia.value, 
    //e.login.value = e.login.value,
    //e.moneda.value = e.moneda.value,
    //e.sine_nac.value = e.sine_nac.value,
    //e.sine_int.value = e.sine_int.value, 
    //e.device_nac.value = e.device_nac.value,
    //e.device_int.value = e.device_int.value,



  //MANDO LAS VARIABLES POR METODO POST A LA URL ESPECIFICADA
  e.action = "disponibilidad.php", e.method = "post", e.submit()
  //e.action = "https://internacionales.conviasa.aero/sistema/pago_agencias/server/disponibilidad.php", e.method = "post", e.submit()
